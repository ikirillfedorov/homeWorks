...
//  HW StringTest
//
//  Created by Kirill Fedorov on 13/02/2019.
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    NSString *firstHalf = [text substringToIndex:text.length / 2];
    NSString *secondHalf = [text substringFromIndex:text.length / 2];
    
    NSLog(@"first half = %@", firstHalf);
    NSLog(@"second half = %@", secondHalf);
    ...
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF CONTAINS %@", @"NSString"]; 
    NSArray *sentanceArray = [array filteredArrayUsingPredicate:predicate];
    ...
    NSMutableArray *reversedSentenceArray = [[NSMutableArray alloc] init];
    
    for (NSString *senctence in sentanceArray) {
        NSArray *wordArray = [senctence componentsSeparatedByString:@" "];
        NSArray *reversedWordArray = [[wordArray reverseObjectEnumerator] allObjects];
        NSString *reversedSenctenceString = [reversedWordArray componentsJoinedByString:@" "];
        [reversedSentenceArray addObject:reversedSenctenceString];
    }
    NSLog(@"%@", reversedSentenceArray);
    ...
    for (int i = 0; i <= [newText length] - 1; i++) {
        char letter = [newText characterAtIndex:i];
        if (![set characterIsMember:letter]) {
